    ...
    .then(dataD => {
      if (true) {
          ...
          for(i = 0; i < dataD.result.comment.length && i < 5; i++) {
            outD += '<div><table frame=void width="100%" border="1" cellpadding="0" cellspacing="0" style="table-layout:fixed"><tr><td weight="50px"><img height="100" src="https://images.weserv.nl/?url=' + dataD.result.comment[i].item.thumbnail.substring(7) + '"></td><td weight="10px"></td><td><a target="_blank" href="https://movie.douban.com/subject/' + dataD.result.comment[i].item.douban_id + '/"><font size="4"><b>' +'· ' + dataD.result.comment[i].item.title + '</b></font></a><div>'
            switch(_type){
              case 0:
